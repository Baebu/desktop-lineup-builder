...
class ImportMixin:
        ...
        with dpg.window(tag=win_tag, label="Import Event", modal=True,
            ...
            def _import(s, a, _pi=paste_tag, _st=status_text, _wt=win_tag):
                ...
                if getattr(self, "_is_dirty", False):
                    ...
                    confirm(
                        "You have unsaved changes. Discard them to import this event?",
                        on_confirm=_do_import,
                        title="Unsaved Changes",
                        confirm_label="Discard & Import",
                        danger=True
                    )
                else:
                    _do_import()
    ...
    def _parse_discord(self, lines: list[str]) -> dict | None:
        ...
        dj_div = self.settings.get("time_dj_divider", " | ").strip() or "|"
        dj_div_esc = re.escape(dj_div)
        genre_div = self.settings.get("genre_divider", " // ").strip() or "//"
        ...
        for line in lines:
            ...
            m = re.match(fr'^<t:(\d+):t>\s*(?:{dj_div_esc}|\|)?\s*(.+)$', line)
            if m:
                unix = int(m.group(1))
            ...
            if (genre_div in line or '//' in line) and not in_lineup:
                genre_text = re.sub(r'^#+\s*', '', line).strip()
                active_div = genre_div if genre_div in genre_text else '//'
                parsed["genres"] =[g.strip() for g in genre_text.split(active_div) if g.strip()]
                continue
    ...
    def _parse_plain(self, lines: list[str]) -> dict | None:
        ...
        for line in lines:
            ...
            # ── Genre line: "Genre1 // Genre2" ───────────────────────────
            if genre_div in line or '//' in line:
                active_div = genre_div if genre_div in line else '//'
                parsed["genres"] =[g.strip() for g in line.split(active_div) if g.strip()]
                continue
    ...
    def _split_title_vol(self, candidate: str, parsed: dict):
        """Split 'Event Title VOL.3' into separate title and vol fields.

        Matches the format produced by ``EventSnapshot.full_title`` with custom prefixes.
        ...
        m = None
        if vol_prefix:
            esc_prefix = re.escape(vol_prefix)
            m = re.search(rf'{esc_prefix}(\d+)\s*$', candidate, re.IGNORECASE)
            
        if not m:
            # Fallback to common patterns if exact match fails
            m = re.search(r'(?:\s+VOL\.|\s+#|\s+\.|\s+)(\d+)\s*$', candidate, re.IGNORECASE)
